...
class CartClass {
  static async addToCart(req, res, next) {
    try {
      ...
      const foundFood = await Dishes.findById(foodToAddToCart)
    ...
    try {
      ...
      if (findCart.userId.toString() == userId._id.toString() && findCart.qty.toString() !== newQty.toString()) {
        if ( newQty <= 0 || null || undefined ) {
          return res.status(501).json("Item can not be less than ONE, so not implemented")
        }
        if (newQty >= 1) {
  ...
  static async removeFromCart(req, res, next) {
    try {
      ...
      const checkCart = findUsersCart.filter((eachItemInUserCart) => {
        return eachItemInUserCart._id.toString() === userId.toString()
      })
