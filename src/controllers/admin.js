...
class controller {
    static async newFood(req, res, next) {
        ...
        try {
            ...
            const foundName = await Dishes.findOne({"name": name})
    
            if (foundName) {
                const err = new Error()
                err.name = "Not Acceptable"
                err.status = 406
                err.message = "This food name is already present"
                throw err
            }
  
            const food = new Dishes({
                name,
                description,
                price,
                category,
    ...
    static async deleteFood(req, res, next) {
      try {
        ...
        let foundFood = await Dishes.findOne({_id: foodId})
        if (!foundFood) {
          const err = new Error()
          err.name = "Not Found"
          err.status = 404
    ...
    static async makeAdmin(req, res, next) {
      try {
        if(req.user.role === "admin") {
          ...
          if(req.user.role !== "admin") {
            throw new Error("sorry, you can't access this function")
          }
        }
      } catch(error) {
        next(error)
    ...
    static async makeFoodAvailable(req, res, next) {
      try {
        ...
        const findFood = await Dishes.findOne({name: foodToMakeAvaiableOrNot})
